...
write.csv(as.data.frame(res), file = './GSEA/fgsea_gbs_rec_vs_control_SYMBOL.csv')
...
write.csv(as.data.frame(DE_down), file = './genes_DE/gbs_rec/transcript_count_gbs_rec_DOWN.csv')
...
write.csv(as.data.frame(DE_up), file = './genes_DE/gbs_rec/transcript_count_gbs_rec_UP.csv')
...
plotMA(res, xlim=xlim, ylim=ylim, main="Guillain-Barré Recuperados vs Controles \n(pdaj < 0.05 + IHW)")
plotMA(resLFC, xlim=xlim, ylim=ylim, main="Guillain-Barré Recuperados vs Controles \n(LFC)")
plotMA(res001, xlim=xlim, ylim=ylim, main="Guillain-Barré Recuperados vs Controles \n(padj < 0.01)")
...
xlim <- c(1,1e5); ylim <- c(-3,3)
plotMA(resLFC, xlim=xlim, ylim=ylim, main="Guillain-Barré Recuperados vs Control \nLFC (apeglm)")
plotMA(resNorm, xlim=xlim, ylim=ylim, main="Guillain-Barré Recuperados vs Control \n(Normalizados)")
plotMA(resAsh, xlim=xlim, ylim=ylim, main="Guillain-Barré Recuperados vs Control \n(Adaptative Shrinkage Estimator-ASHR)")
...
########################################### SELEÇÃO GENES UP e DOWN RES 2 ###########################################
contr_gbs_rec <- as.data.frame(res2)
...
resLFC <- lfcShrink(dds, coef = 'condition_gbs_rec_vs_control', type = 'apeglm', res = res2)
res001 <- results(ddssva, filterFun=ihw, alpha=0.01)

# Agora, observar os plots juntos
# Versão 1: ma_all_v1.jpeg
par(mfrow=c(2,3), mar=c(4,4,4,2))
xlim <- c(1,1e5); ylim <- c(-3,3)
plotMA(res2, xlim=xlim, ylim=ylim, main="Febre Zika vs Controles \n(pdaj < 0.05 + IHW)")
