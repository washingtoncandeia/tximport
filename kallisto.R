...
write.table(samples_info, './tables/zika/condition_zika_vs_control.txt', sep = '\t')
...
samples <- read.table('./tables/zika/condition_zika_vs_control.txt', header = TRUE, row.names = 1)
head(samples, 12)
...
write.csv(as.data.frame(res), file = './GSEA/zika/GSEA_zika_vs_control.csv')
...
summary(resOrdered)
# Criar csv (pode ser usado em fgsea)
write.csv(as.data.frame(resOrdered), file = './GSEA/zika/GSEA_zika_vs_control_resOrdered.csv')
read.table('./GSEA/zika/GSEA_zika_vs_control_resOrdered.csv', header = TRUE,  sep = '\t',  stringsAsFactors = FALSE)
...
plotMA(res, xlim=xlim, ylim=ylim, main="Febre Zika vs Control \n(pdaj < 0.1)")
plotMA(resOrdered, xlim=xlim, ylim=ylim, main="Febre Zika vs Control \n(Reordenado por menor pvalue)")
plotMA(resLFC, xlim=xlim, ylim=ylim, main="Febre Zika vs Control \n(LFC)")
plotMA(resIHW, xlim=xlim, ylim=ylim, main="Febre Zika vs Control \n(IHW)")
plotMA(res05, xlim=xlim, ylim=ylim, main="Febre Zika vs Control \n(padj < 0.05)")
...
# Pode-se especificar o coef com número ou string.
