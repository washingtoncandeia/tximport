...
# Obtendo .CSV contendo Transcripts IDs e Gene IDs por biomaRt Ensembl.
mart <- useMart(biomart = "ensembl", dataset = "hsapiens_gene_ensembl")

results <- getBM(attributes = c("ensembl_transcript_id_version", "ensembl_gene_id"), mart = mart)

tx2gene <- results[, 1:2]
# https://github.com/BarryD237/D-O-Connor
...
# Abundance countsFromAbundance = scaledTPM
txi.kallisto.cfa <- tximport(files,
                             type = 'kallisto',
                             tx2gene = t2g, 
                             countsFromAbundance="scaledTPM")

# Abundance countsFromAbundance = lengthScaledTPM
txi.cfa.len <- tximport(files, 
                        type = "kallisto", 
                        tx2gene = t2g, 
...
# Pacote: fgsea
# Formato de arquivos input de GSEA (MSigDB): Gene sets in gmt format.
dds <- DESeq(dds)
# A função results gera tabelas de resultados.
