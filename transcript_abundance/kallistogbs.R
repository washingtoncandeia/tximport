...
res.tx <- read_csv('./GSEA/gbs/condition_gbs_rec_vs_control.csv')

my_data <- as_tibble(res.tx)
my_data

res.tx <- my_data %>% rename("ensembl_id" = X1)
res.tx
...
# https://support.bioconductor.org/p/81012/
...
res.tx <- inner_join(res.tx, ens2symbol, by = c('ensembl_id'='ENSEMBL'))
...
write.csv(as.data.frame(res), file = './GSEA/gbs/condition_gbs_rec_vs_control.csv')
